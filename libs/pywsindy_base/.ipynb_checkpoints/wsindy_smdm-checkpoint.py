...
class WSINDy:
  ...
  def optimal_support(self, d, changepoint, F_root):
    ...
    lower_bnd = (np.sqrt(3)*Nd*self.tau_hat)/(2*np.pi*k)
    upper_bnd = lower_bnd*np.sqrt(1 - (8/np.sqrt(3))*np.log(self.tau))
    md = int(scipy.optimize.brentq(F_root, lower_bnd, upper_bnd, args=(k, Nd, self.tau_hat, self.tau)))
    md = min(md, int((Nd-1)/2))
