...
# the next three functions are used to get polynomial tags
def weak_compositions(N, k):
    ...
    for bars in combinations(range(N + k - 1), k - 1):
        cuts = (-1,) + bars + (N + k - 1,)
        rows.append(
            [cuts[i + 1] - cuts[i] - 1 for i in range(k)]
        )

    return np.asarray(rows, dtype=int)

def partition_nk(N, k):
    if N > 1:
        return weak_compositions(N, k)
    elif N == 1:
...
def get_tags(
    ...
    """
    Python version of MATLAB get_tags().
    """
    ...
    poly_blocks = [partition_nk(p, n) for p in polys]

    if poly_blocks:
        tags = np.vstack(poly_blocks)
    else:
        tags = np.empty((0, n), dtype=int)
    ...
    if boolT is None:
        ...
        mask = np.array([boolT(tag) for tag in tags], dtype=bool)

    tags = tags[mask]
    tags = unique_rows_stable(tags)
    ...
    if trigs.size:
        ...
        if trigs.ndim == 1:
            ...
            for t in trigs:
                trig_rows.append(-1j * t * np.eye(n))
                trig_rows.append(+1j * t * np.eye(n))

        else:

            for row in trigs:
                trig_rows.append(-1j * np.diag(row))
                trig_rows.append(+1j * np.diag(row))
    ...
    if not uni:
        return [tags[i:i+1] for i in range(tags.shape[0])]
