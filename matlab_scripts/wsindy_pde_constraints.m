%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This script demonstrates how to incorporate linear constraints into WSINDy to PDE.
...
lib = get_lib(Uobj,polys,trigs,x_diffs, {}, {}, {});
...
[Ai_cell,bi_cell] = stable_diffusion_constraints(WS,-0.4);

linregargs = cellfun(@(A,b) {'Aineq', A, 'bineq', b, 'verbose', 'None'}, Ai_cell, bi_cell, 'un', 0 );

tic;
[WS,loss_wsindy,its,G,b] = WS_opt().MSTLS_0(WS,'lambdas',lambdas,'M_diag',Mscale,'toggle_jointthresh',threshold_scheme,'alpha',[], 'linregargs', linregargs);
runtime = toc;
